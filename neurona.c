...
void neurona_conectar( Neurona* n, uint32 neurona_capa_siguiente ){

	printf("Conectando neurona a neurona %u \n", neurona_capa_siguiente);
	n->n_conexiones += 1;
...
void neurona_mostrar( Neurona* n ){
	...
	for(int i = 0; i < n->n_conexiones; ++i){
		printf("Conexion %u: conectado con neurona %u y peso %f\n", i, n->conexiones[i], n->pesos[i] );
	}
...
void neurona_test(){
	...
	neurona_conectar( &neurona, 3);
	neurona_conectar( &neurona, 7 );
	neurona_conectar( &neurona, 11);
	neurona_conectar( &neurona, 15);
